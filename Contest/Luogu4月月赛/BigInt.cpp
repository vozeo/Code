...
void convert_s(const __base_t &a, char *s) {
    ...
    for (int i = a.size() - 2; ~i; --i) ptr += sprintf(ptr, "%09d", a[i]);
    strcpy(s, buffer);
}
...
LL convert_i(const __base_t &a) {
    LL b = 1, c = 0;
    for (int i = 0; i < (int)a.size(); ++i) c += a[i] * b, b = b * MOD;
    return c;
}
...
__base_t addi(const __base_t &a, const __base_t &b) {
    ...
    __base_t c = a;
    c.push_back(0);
    for (int i = 0; i < (int)b.size() || c[i] >= MOD; ++i) {
        if (i < (int)b.size())
            c[i] += b[i];
        if (c[i] >= MOD)
            c[i] -= MOD, c[i + 1]++;
...
__base_t subt(const __base_t &a, const __base_t &b) {
    __base_t c = a;
    for (int i = 0; i < (int)a.size() && (i < (int)b.size() || c[i] < 0); ++i) {
        if (i < (int)b.size())
            c[i] -= b[i];
        if (c[i] < 0)
            c[i] += MOD, c[i + 1]--;
...
#define __get_val(x, y, z)                                             \
    (10ull * MOD * ((y) + 1 < (z) ? x[(y) + 1] : 0) + 10ull * x[(y)] + \
     ((y)-1 >= 0 ? x[(y)-1] : 0) / (MOD / 10))
void trial_division(const __base_t &a, const __base_t &b, __base_t &c, __base_t &d) {
    ...
    for (int i = a.size() - b.size(); ~i; --i) {
        int p;
        while (p = __get_val(d, i + (int)b.size() - 1, d.size()) /
                   (__get_val(b, (int)b.size() - 1, b.size()) + 1),
               p)
...
struct BigInt {
    ...
    LL to_int() const { return (is_neg ? -1 : 1) * __BigInt_b::convert_i(data); }
...
#define __ext_opt(opt, __opt)                          \
    template <typename T>                              \
    BigInt operator opt(const T &a, const BigInt &b) { \
        return (BigInt)(a)opt b;                       \
    }                                                  \
    template <typename T>                              \
    BigInt operator opt(const BigInt &a, const T &b) { \
        return a opt(BigInt)(b);                       \
    }                                                  \
    template <typename T>                              \
    BigInt &BigInt::operator __opt(const T &b) {       \
        *this = *this opt(BigInt) b;                   \
        return *this;                                  \
    }

__ext_opt(+, +=) __ext_opt(-, -=) __ext_opt(*, *=) __ext_opt(/, /=) __ext_opt(%, %=)
