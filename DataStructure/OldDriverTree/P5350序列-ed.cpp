...
set<Node> s;
Iter itl, itr, itl1, itl2, itr1, itr2;
...
void copy(int l1, int r1, int l2, int r2) {
    t = l2 - l1, itr2 = split(r2 + 1), itl2 = split(l2);
    s.erase(itl2, itr2);
    itl1 = split(l1);
    if (r1 + 1 != l2)
        split(r1 + 1);
...
void swp(int l1, int r1, int l2, int r2) {
    ...
    t = l2 - l1, top = q;
    itr1 = split(r1 + 1), itl1 = split(l1), it = itl1;
    for (; it != itr1; ++it)
        *(++top) = Node(it->l + t, it->r + t, it->v);
    s.erase(itl1, itr1);
    itr2 = split(r2 + 1), itl2 = split(l2), it = itl2;
    for (; it != itr2; ++it)
...
void rev(int l, int r) {
    t = l + r, top = q;
    itr = split(r + 1), itl = split(l), it = itl;
    for (; it != itr; ++it)
        *(++top) = Node(t - it->r, t - it->l, it->v);
    s.erase(itl, itr);
    while (top > q)
        s.insert(*(top--));
}
