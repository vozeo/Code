...
struct SegT {
    ...
    SegT *lc, *rc;
    SegT() {}
    SegT(int v) : sum(v), lc(NULL), rc(NULL) {}
    SegT(SegT *l, SegT *r) : lc(l), rc(r) {
        maintain();
    }
...
SegT *build(int l, int r) {
    ...
    int mid = (l + r) >> 1;
    return new (++tcur) SegT(build(l, mid), build(mid + 1, r));
}
...
void modifyR(int l, int r, int w) {
    ...
    while (x->top != y->top) {
        if (x->top->dep < y->top->dep)
            swap(x, y);
        segt->modify(x->top->dfn, y->top->dfn, 1, n, w);
        x = x->top->fa;
    }
    if (x->dep > y->dep)
        swap(x, y);
...
int queryR(int l, int r) {
    ...
    while (x->top != y->top) {
        if (x->top->dep < y->top->dep)
            swap(x, y);
        ret = 0;
        segt->query(x->top->dfn, y->top->dfn, 1, n);
        ans += ret;
        x = x->top->fa;
    }
    if (x->dep > y->dep)
